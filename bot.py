...
async def smart_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "scheme" in text or "term" in text or "grade" in text:
        await scheme(update, context)
        return

    if "lesson plan" in text:
        await lessonplan(update, context)
        return
...
app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, smart_mode))
